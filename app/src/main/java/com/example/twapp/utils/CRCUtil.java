 ...
 * ━━━━━━神兽出没━━━━━━
 *
 * @author :孙天伟 in  2017/9/27   13:26.
 *         联系方式:QQ:420401567
 *         功能描述:  crc 16  校验  结果返回0
 */
...
public class CRCUtil {
	 ...
	 */
    private static int crc16_ccitt_1021[] = {0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129, 0xa14a,
    ...
    /**
     *
     * @param reg_init  计算CRC校验的初值
     * @param bytes 需要计算的数据
     * @return
     */

    public static byte[] GetCRC(int reg_init, byte[] bytes) {
        int crc = reg_init;  //初始值
        for (byte b : bytes) {
            crc = (crc << 8) ^ crc16_ccitt_1021[((crc >>> 8) ^ b) & 0xff];
        }
        byte[] b = new byte[2];
        b[0] = (byte) (crc >> 8);
        b[1] = (byte) (crc >> 0);
